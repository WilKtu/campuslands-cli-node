...
const RUTA_DATA = path.join(__dirname, '..', 'data', 'can...');

export async function LeerCampus() {
    try {
...
export async function agregar(nombre, stack) {
    if (!nombre || !stack){
    ...
    const nuevoCamper = {
        ...
        nombre : nombre.trin(),stack: stack.trin(),
...
export async function listarCamper() {
    return await LeerCampers();
}
export async function buscarCamperPorNombre(termino) {
    const campers = await LeerCampers();
    return campers.filter(c => c.nombre.toLowerCase().includes(ternimo.toLowerCase()));
}
